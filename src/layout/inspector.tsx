...
/** The origin inspector — the "Components" tab: what the screen REALLY owes to the kit.
 *
 ...
 *   • the chrome (sidebar, ambient background, the bar, the panel) is provided by the
 *     skeleton and therefore stays out of the count — it says nothing about how THIS
 ...
 *
 *  Until 2026-09-08 it was a bottom sheet behind a button of the bar; it is now one tab
 *  of the side panel, so the two columns became one — the panel is 380 px wide. */
...
export const InspectorBody = ({ active }: { active: boolean }) => {
  ...
  useEffect(() => {
    ...
    document.documentElement.toggleAttribute("data-42-inspect", on)
  ...
  const silent = active ? IMPORTS_KIT.filter((n) => !seen.has(n)) : []

  return (
    <div className="flex flex-col gap-4 px-3 py-3" aria-label="Origin of the screen's components">
      <div className="flex flex-col gap-1">
        <span className="font-mono font-semibold text-sm text-white">{ratio}% from the kit</span>
        <span className="text-gray-dark-400 text-xs">
          {nKit} @42/ui-react element{nKit > 1 ? "s" : ""} · {nHand} written by hand
        </span>
        <label className="mt-1 flex cursor-pointer items-center gap-2 text-gray-dark-400 text-xs">
          <input
            type="checkbox"
            checked={highlight}
      ...
      {silent.length > 0 && (
        <div className="flex flex-col gap-1 rounded-md border border-gray-dark-800 bg-white/2 px-3 py-2">
          <span className="text-gray-dark-300 text-xs">
            ...
            components really are used by the screen, but they do not forward their props down
            to the DOM: they escape both the count and the highlighting.
          </span>
      ...
      <p className="text-gray-dark-500 text-xs leading-relaxed">
        ...
        <code className="font-mono">div</code>s around a card weigh as much as the card. The
        chrome (sidebar, background, bar, this panel) comes from the skeleton and is not
        counted. The origin is read from the source code at compile time, not guessed in the
        DOM.
      </p>
    </div>
  )
