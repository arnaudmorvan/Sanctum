import { FieldShell, fieldA11yProps } from './chunk-FSC5UYO3.js';
...
var segItem = cva(
  [
    "font-medium",
    "cursor-pointer rounded-input text-center whitespace-nowrap select-none",
  ...
  ],
  {
    variants: {
      ...
      size: {
        // No vertical padding of its own — the tray's `min-h` (below) sets the
        // box height and this item stretches to fill it (the tray's default
        // `align-items: stretch` propagates all the way down), so the label
        // just needs to center itself inside that taller box. This matches an
        // `Input` / `NumberInput`'s exact box height at the same size.
        xs: "flex items-center justify-center px-1 text-xs",
        sm: "flex items-center justify-center px-1.5 text-sm",
        md: "flex items-center justify-center px-2 text-md",
        lg: "flex items-center justify-center px-3 text-lg",
        xl: "flex items-center justify-center px-4 text-xl"
      }
    },
...
var segIndicator = cva("z-0", {
  variants: {
    variant: {
      filled: "bg-(--c-solid) rounded-xxs",
      light: "bg-(--c-soft) border border-(--c-soft-hover) rounded-xxs",
      outline: "bg-transparent border border-(--c-solid) rounded-input",
      subtle: "bg-(--c-soft-hover) rounded-input",
      default: "bg-(--c-solid) rounded-xxs"
    }
  },
...
var segTray = cva(
  ...
  ],
  {
    variants: {
      ...
      size: {
        // The exact `min-h` `inputShell` (input.tsx) uses at the same size —
        // so a `SegmentGroup` sits flush with an `Input` / `NumberInput` of
        // the same size in a row (see `TimeRow`, which composes both).
        xs: "gap-0 min-h-7",
        sm: "gap-0 min-h-8",
        md: "gap-0 min-h-9",
        lg: "gap-0 min-h-10",
...
export { SegmentGroup };
//# sourceMappingURL=chunk-OWMQ7H6E.js.map
//# sourceMappingURL=chunk-OWMQ7H6E.js.map