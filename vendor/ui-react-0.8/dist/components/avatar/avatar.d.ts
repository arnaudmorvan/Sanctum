 ...
 *
 * `Avatar.Group` overlaps a row of standalone `Avatar`s (Mantine-style
 * stacked avatars), punching a transparent notch into each one where its
 * successor overlaps it — true see-through separation instead of a painted
 * border, so it reads correctly over any page background. It's pure CSS
 * layout, like `ButtonGroup` — it doesn't resize or restyle its children, so
 * set `size`/`color` per `Avatar`.
 */
...
/** `Avatar.Group` — see the doc comment above the `Avatar` component itself. */
export declare const Avatar: (({ src, alt, size, radius, color, palette, name, display, children, className, classNames, style, testId, ...rest }: AvatarProps) => import("react").JSX.Element) & {
    Group: ({ spacing, className, style, testId, ...rest }: AvatarGroupProps) => import("react").JSX.Element;
};
export {};
