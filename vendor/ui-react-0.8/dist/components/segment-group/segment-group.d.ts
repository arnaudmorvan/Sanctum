 ...
 * `data-color`, so every kit color tints for free, `default` included — its pill
 * falls back to solid black/white with no color set. A per-item `color`
 * overrides the group palette for whichever segment is selected, signalled by
 * `data-tinted` (which flips the `default` variant's checked label to
 * `--c-on-solid` for contrast against the now-colored pill).
 *
...
export type SegmentGroupProps<Data extends SegmentGroupData = SegmentGroupData> = Pick<FieldProps, "label" | "description" | "error" | "required"> & WithTestId & {
    ...
    readOnly?: boolean;
    /** `xs`–`xl` match the exact box height of an `Input` / `NumberInput` at
     *  that size, so a `SegmentGroup` composes flush inline next to one (see
     *  `TimeInput`'s AM/PM toggle). */
