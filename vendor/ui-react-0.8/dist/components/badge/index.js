...
var badgeVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      // Icon size ([&_svg]:size-*) isn't in the Figma spec — scaled by eye
      // to sit proportionate to each step's min-h/padding/text, same
      // convention as Button's own size variant.
      size: {
        xs: "min-h-5 px-0.5 text-[10px] leading-2.5 gap-0.5 rounded-sm [&_svg]:size-2.5",
        sm: "min-h-5.5 px-1 text-xs gap-0.75 rounded-sm [&_svg]:size-3",
        md: "min-h-6 px-1.5 text-sm gap-1.5 rounded-sm [&_svg]:size-3",
        lg: "min-h-7 px-2 text-md gap-2 rounded-sm [&_svg]:size-3.5",
        xl: "min-h-8 px-2.5 text-lg gap-3.5 rounded-sm [&_svg]:size-4"
      }
    },
    defaultVariants: {
      variant: "light",
