 ...
 *   - `description` (helper text) sits between the label and the control.
 *     `error` sits below the control and renders independently of it — both
 *     can be visible at once. Passing `error` flips the field to `invalid`,
 *     which both styles field-aware controls red and associates the message.
...
/**
 * `aria-describedby` / `aria-errormessage` / `aria-labelledby` for a control nested
 * in a `Field`. Points `aria-describedby` at the error text id directly once
 * `invalid`, rather than trusting `field.ariaDescribedby`'s own DOM-presence-tracked
 * inclusion of it (a `MutationObserver` check that doesn't reliably catch up for
 * every control) — and adds `aria-errormessage`, which Ark's field getters never
 * emit for any control. `aria-labelledby` points at the id already rendered on
 * `Field`'s own `<label>`: needed because `NumberInput`, `PasswordInput`,
 * `TagsInput`, `Select`, and `SegmentGroup`'s real interactive element isn't
 * `Field`'s own `Ark.Input`, so it never otherwise carries an id matching the
 * label's `htmlFor`.
 */
