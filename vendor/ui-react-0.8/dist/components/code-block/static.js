...
import { CodeBlockChrome } from './chrome.js';
...
async function CodeBlockStatic({
  ...
  const notation = parseNotation(code);
  const preClassName = cn("overflow-x-auto py-3 text-sm leading-relaxed font-mono", classNames?.pre);
  const [headHtml, tailHtml] = await Promise.all([
    highlightCode(headCode, { lang: language, themes, tags, gutters, notation, transformers }),
    isCollapsible ? highlightCode(tailCode, {
      lang: language,
      themes,
      tags,
      gutters,
  ...
  return /* @__PURE__ */ jsx(
    CodeBlockChrome,
    {
      ...
      className,
      classNames,
      headContent: /* @__PURE__ */ jsx(StaticPre, { html: headHtml, className: preClassName }),
      tailContent: tailHtml != null && /* @__PURE__ */ jsx(StaticPre, { html: tailHtml, className: preClassName }),
      ...chromeProps
    }
  );
}

export { CodeBlockStatic };
