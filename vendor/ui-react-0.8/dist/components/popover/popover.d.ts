...
export type PopoverProps = Omit<ArkPopover.RootProps, "positioning"> & {
    ...
    /** Hide the panel (`visibility: hidden`, no pointer events) once the
     *  trigger is fully clipped by its own scrolling ancestor, and restore it
     *  once back in view — Floating UI's `hide()` middleware
     *  (`strategy: "referenceHidden"`), already wired into Ark's positioning
     *  engine, just not exposed until now. Rides the positioner's existing
     *  scroll/resize listeners, so no extra observer is needed. Default `true`
     *  — a panel that can't see its own trigger is never in a state worth
     *  keeping visible; pass `false` to restore the old "keep following the
     *  trigger's last known position" behavior. */
...
export type PopoverAnchorProps = ArkPopover.AnchorProps;
export declare const Popover: {
    Root: ({ width, position, gutter, offset, hideWhenDetached, withinPortal, container, children, onOpenChange, onClose, ...rest }: PopoverProps) => import("react").JSX.Element;
    Trigger: import("react").ForwardRefExoticComponent<ArkPopover.TriggerProps & import("react").RefAttributes<HTMLButtonElement>>;
    /** Positions the panel without attaching any click/keyboard trigger
     *  semantics — for a host that already owns its own click resolution
     *  (e.g. `Schedule`'s drag-vs-click gesture) and only needs the panel to
     *  float relative to a given element. */
    Anchor: import("react").ForwardRefExoticComponent<ArkPopover.AnchorProps & import("react").RefAttributes<HTMLDivElement>>;
