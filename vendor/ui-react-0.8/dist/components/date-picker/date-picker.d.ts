...
export type DatePickerProps<Range extends boolean = false> = FormControlProps<Range extends true ? DateRangeValue : Date | null, HTMLButtonElement | HTMLInputElement> & Pick<FieldProps, "label" | "description" | "error" | "required"> & WithTestId & {
    ...
    /** Renders Ark's own `Input` anatomy part (typing, arrow-key navigation, blur-to-parse,
     *  `Enter`-to-commit) alongside the calendar-icon trigger, instead of the plain button-only
     *  trigger. Opt-in — default `false` keeps today's click-to-open button unchanged.
     *
     *  Only takes effect for a single, non-`withTime` date: `range` (cross-field validation across
     *  two `Input`s) and `withTime` (the default date-only format the machine parses typed text
     *  against doesn't include a time portion) are both out of scope for this prop today and fall
     *  back to the button trigger — see the component's own PR notes for the deferral. */
    typeable?: boolean;
    min?: Date;
    max?: Date;
...
declare const TimePanelRow: ({ value, onSetTime, label, disabled, hourCycle, minuteStep, withSeconds, placeholder, locale, translations, size, variant, classNames, }: TimePanelRowProps) => import("react").JSX.Element;
